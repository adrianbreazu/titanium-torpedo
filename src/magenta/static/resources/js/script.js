...
(function() {
    var $canvas_dashboard = $('#canvas_dashboard');
    var sensor_data_array = [];
    var sensor_label_array = [];

    $.ajax({
        url: 'http://127.0.0.1:8080/sensors_actuators/getPeriodIntervalData/',
        type: "POST",
        data: '{ "startPeriod": "2013-01-01", "endPeriod" : "2019-01-31", "id" : "2"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function(message) {
            var msg_array = message['read'];
            $.each(msg_array, function(index, value) {
                sensor_data_array.push(value['value']);
                sensor_label_array.push(value['datetime']);
            });
        },
        error: function(error) {
            console.log("Error in get period interval data");
            console.log(error);
        }
    });

    var new_graph = new Chart($canvas_dashboard, {
        type: 'line',
        data: {
            labels: sensor_label_array,
            datasets: [{
                label: "Senor 1 name here",
                backgroundColor: "rgba(38, 185, 154, 0.38)",
                data: sensor_data_array
            }],
        }
    });
})();